...
show_usage() {
    ...
    echo "  -m, --no-maui-build       Skip MAUI build step"
...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        -m|--no-maui-build) SKIP_MAUI_BUILD=true ;;
...
if [ "$SKIP_MAUI_BUILD" = false ]; then
  echo "Restoring MauiDemoApp"
  dotnet restore MauiDemoApp/MauiDemoApp.csproj

  echo "Building MauiDemoApp"
  if [ "$BUILD_PLATFORM" = "ios" ]; then
    dotnet build MauiDemoApp/MauiDemoApp.csproj --framework net9.0-ios
  elif [ "$BUILD_PLATFORM" = "android" ]; then
    dotnet build MauiDemoApp/MauiDemoApp.csproj --framework net9.0-android
  else
